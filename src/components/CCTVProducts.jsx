import { Box, Flex, Text, Image, Container, Heading, HStack, Button, IconButton } from "@chakra-ui/react";
import { FaChevronRight, FaBuilding, FaChevronLeft } from "react-icons/fa";
import { useState, useRef, useEffect } from "react";
import { Link } from "react-router-dom";
import { topBrands, cctvCategories } from "../data/data";
...
export default function CCTVProducts() {
  ...
  return (
    <Container maxW="container.xl" py={{ base: 6, md: 8 }} px={{ base: 2, md: 4 }} bg="rgb(255,254,254)" borderRadius={{ base: '16px', md: '24px' }}>
      <Box mb={10}>
        <Heading
          as="h2"
            ...
            <Flex gap={4} display="inline-flex" py={2}>
              {topBrands.map((brand, index) => (
                <Box
                  ...
                  gap={2}
                  as={Link}
                  to={`/cctv?brand=${encodeURIComponent(brand.name)}`}
                  textDecoration="none"
        ...
        <Box position="relative">
          ...
          >
            <Flex gap={4} display="inline-flex">
              {cctvCategories.map((category, index) => (
                <Box
                  key={index}
                  className="scroll-card"
